...
{
    return Uart16550WriteSerialByte(Adapter, Byte);
}
...
Routine Description:
    ...
    Synchronously read a single byte from the serial port or time out.  This
    will immediately timeout if there is not a byte on the serial FIFO.
...
Return Value:
    ...
    STATUS_SUCCESS on a successful read from the serial port.  
    STATUS_IO_TIMEOUT if there is no data available.

--*/

{
    return Uart16550ReadSerialByte(Adapter, Byte);
}
...
{
    return Uart16550InitializeController(KdNet);
}
...
{
    Uart16550ShutdownController(Adapter);
    return;
}
...
{
    return Uart16550GetContextSize(Device);
}
    ...
    )
{
    ...
    Device->Memory.Length = Uart16550GetContextSize(Device);
